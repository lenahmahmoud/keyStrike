...
const HowItWorks = () => {
  return (
    <section className="bg-bgdark  px-8 py-20 mt-10 text-white">
      <div className="w-[70%] mx-auto text-center">
        <h1 className="text-3xl font-bold text-grLight">How It Works</h1>
        <p className="mt-3 text-grDark">
          ...
          {steps.map(({ number, icon: Icon, title, description }) => (
            ...
            >
              <div className="flex items-center gap-3 ">
                <div className="bg-white/5 rounded-lg p-2 flex items-center">
                  <Icon className="w-5 h-5 text-accent" />
                </div>
              
              <h3 className=" font-semibold text-accent ">{title}</h3>
                </div>
              <p className="mt-2 text-sm text-grLight leading-relaxed">
                {description}
              </p>
            </div>
          ))}
        ...
        <button className="mt-10 bg-accent text-dark font-semibold px-6 py-2.5 rounded-lg hover:brightness-95 cursor-pointer">
          Got it, let's go
        </button>
      </div>
    </section>
